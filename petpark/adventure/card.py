...
def card_html(player, key, equipment=False):
    ...
    details = (f'<div class="power-formula"><span>战力构成</span>'
               ...
               f'<small>灵宠战力 ×{bd["pet_ratio"]:.1%}（随修士等级，满级 {bd["pet_ratio_max"]:.0%}；上限 本体×{bd["pet_cap_ratio"]:g}） · 坐骑战力 ×{bd["mount_ratio"]:.0%} · 道侣 ×{bd["partner"]:.2f} · 洞天 ×{bd["heaven_margin"]:.2f}</small></div>') if bd else ""
