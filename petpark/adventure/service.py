    ...
    def _handle(self, group, qq, tokens):
        ...
        if cmd in ("我的修士", "今日修行"):
            ...
            if bd:
                power_lines = (
                    f"总战力 {bd['total']}\n"
                    f"　构成：本体 {bd['hero']} ＋ 灵宠『{bd['pet_name']}』{bd['pet_power']}×{bd['pet_ratio']:.0%}={bd['pet_contrib']}"
                    f"（上限 本体×{bd['pet_cap_ratio']:g}） ＋ 坐骑 {bd['mount_power']}×{bd['mount_ratio']:.0%}={bd['mount_contrib']} ＝ {bd['base']:.1f}\n"
                    f"　再乘：道侣×{bd['partner']:.2f} · 洞天×{bd['heaven_margin']:.2f} → {bd['total']}"
                )
            else:
                power_lines = "总战力 0"
            _tactics = a.get("tactics", [])
            tactics = " · ".join(t + (f"（{c.tactic_effect(t)}）" if c.tactic_effect(t) else "") for t in _tactics) or "无"
            _root = a.get("spirit_root") or "无"
        ...
        if cmd == "道号":
            ...
            if not self.store.remove_item(p, "改名符"):
                return "修改道号需要『改名符』×1（灵石商城购买；此符只用于修士道号，宠物改名用『宠物改名 昵称』消耗改名卡）。"
            a["name"] = arg
        ...
        if cmd == "宗门帮助":
            return ("【宗门】群级共享，一宗一界。\n"
                    ...
                    "宗门捐献灵石(10灵石=1帮贡)升帮贡 · 灵材专用于装备锻造/洗炼，不再捐献。\n"
