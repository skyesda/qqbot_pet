...
# ----------------------------------------------------------------------------
# 自动助手（原「自动修炼」）
# 计费从「天数」改为「次数」：1 张自动助手卡 = ASSISTANT_QUOTA_PER_CARD 次执行额度，
# 每成功执行 1 个任务扣 1 次，额度归零自动停机。
# ----------------------------------------------------------------------------
ASSISTANT_QUOTA_PER_CARD = 500   # 1 张「自动助手卡」提供的执行次数
ASSISTANT_QUOTA_CUSTOM_PET = 1000  # 定制宠物主人的次数池保底（解锁定制与存量迁移均提升至该值，只升不降）
ASSISTANT_MAX_TASKS = 4          # 每只宠物最多勾选的任务数
ASSISTANT_LOG_MAX = 20           # 每只宠物保留的最近执行记录条数（供「助手状态」查看）

# 定制宠物专属的免费任务：这些任务在自动助手里**永久不扣次数**，与月卡、「限时免费」
# 后台窗口都无关，剩余次数为 0 也能照跑。
# 只写 key：「修炼」一个 key 同时覆盖双修 —— 已婚宠物的「修炼」任务会自动改跑『双修』
# （main._assistant_cult_action），故无需单列「双修」。
ASSISTANT_CUSTOM_FREE_TASKS = ("修炼", "幻境寻宝")
...
# 助手任务注册表：(key, 按钮标签, 状态页说明, axis)
#   axis="pet"  → 作用于当前灵宠
#   axis="cult" → 作用于修士角色（player["adventure"]），与灵宠无关
# 列表顺序即按钮面板顺序，也是后台每轮的执行顺序。
#
# ⚠️ 按钮标签必须 ≤2 个汉字：QQ 对按钮 label 卡约 **6 字节** 的硬上限，超出即被截成
#    「首字 + ...」（实测 `神仙劫`→`神...`、`家园收取`→`家...`，而 `砸蛋`/`打工` 完整显示）。
#    这是标签字节数限制、**不是**按钮宽度问题（按钮右侧大片留白仍然截断），所以
#    减少每行按钮数无济于事；也正因如此标签**不能带任何前缀标记**——`⬜砸蛋` 是 9 字节必被截。
#    勾选态改由面板正文的「已选 N/4」一行呈现，见 main._assistant_keyboard。
#    key（第 1 项）不受此限，它是指令参数与存档值，且「助手状态」页直接用 key 显示全名。
ASSISTANT_TASKS: list[tuple[str, str, str, str]] = [
    ("修炼", "修炼", "修炼（已婚自动转双修）", "pet"),
    ("进入副本", "副本", "进入副本（自动挑最高档）", "pet"),
    ("挑战神仙", "神仙", "挑战神仙（自动挑最高档）", "pet"),
    ("外出历练", "历练", "外出历练（修士）", "cult"),
    ("秋冬Boss", "活动", "当前活动 Boss", "pet"),
    ("深渊秘境", "深渊", "深渊秘境", "pet"),
    ("宠物神仙劫", "仙劫", "宠物神仙劫", "pet"),
    ("砸蛋", "砸蛋", "砸蛋", "pet"),
    ("砸蛋十连", "十连", "砸蛋十连（与砸蛋共用冷却）", "pet"),
    ("打工", "打工", "打工", "pet"),
    ("学习", "学习", "学习", "pet"),
    ("洗髓", "洗髓", "洗髓", "pet"),
    ("探险", "探险", "探险", "pet"),
    ("冥想", "冥想", "冥想（需定制宠物）", "pet"),
    ("幻境寻宝", "寻宝", "幻境寻宝（需飞升）", "pet"),
    ("家园收取", "家园", "家园收取", "pet"),
    ("约会", "约会", "道侣之约", "pet"),
    ("修士修炼", "修士", "修士修炼（修士）", "cult"),
    ("修士突破", "突破", "修士突破（修士）", "cult"),
]

# key → 注册项，便于按 key 反查（按钮回调、持久化配置都用 key）
ASSISTANT_TASK_KEYS = [t[0] for t in ASSISTANT_TASKS]
ASSISTANT_TASK_BY_KEY = {t[0]: t for t in ASSISTANT_TASKS}
...
ITEMS = {
    ...
    "自动助手卡": {
        ...
        "category": "道具",
        "usable": True,
        "desc": f"使用后获得 {ASSISTANT_QUOTA_PER_CARD} 次自动助手执行额度。",
        "effect": {"add_assistant_quota": ASSISTANT_QUOTA_PER_CARD},
    },
    # ---- 深渊道具 ----
    "净化药水": {
        "price": 5000,
    ...
    "宠物定制卡": {
        ...
        "category": "道具",
        "usable": True,
        "desc": "开启主宠「定制」权限（自定义名称/图片），并晋升为【超脱】品质、加「定制」标签，并使自动助手次数保底 "
                f"{ASSISTANT_QUOTA_CUSTOM_PET}。用法：『使用 宠物定制卡』。",
...
ITEMS.update({
    ...
    "小经验书": {
        "price": 4000, "currency": CURRENCY_JIFEN, "category": "药品", "usable": True,
        "desc": "使用后给结契灵宠 +1000 经验。",
        "effect": {"companion_exp": 1000},
    },
    "中经验书": {
        "price": 12000, "currency": CURRENCY_JIFEN, "category": "药品", "usable": True,
        "desc": "使用后给结契灵宠 +5000 经验。",
        "effect": {"companion_exp": 5000},
    },
    "大经验书": {
        "price": 25000, "currency": CURRENCY_JIFEN, "category": "药品", "usable": True,
        "desc": "使用后给结契灵宠 +10000 经验。",
        "effect": {"companion_exp": 10000},
    },
    "蓄力丸": {
        "price": 50, "currency": CURRENCY_DIAMOND, "category": "仙丹", "usable": True,
    ...
    "神龙果": {
        "price": 1200, "currency": CURRENCY_DIAMOND, "category": "仙丹", "usable": True,
        "desc": "使用后修为翻倍30天。",
...
MS_DIFFICULTIES = {
    1: {"name": "简单", "size": (6, 6), "mines": 5, "time": 450, "exp": (400, 800), "score": 10},
    2: {"name": "普通", "size": (9, 9), "mines": 12, "time": 900, "exp": (900, 1800), "score": 25},
    3: {"name": "困难", "size": (12, 12), "mines": 24, "time": 1350, "exp": (1800, 3200), "score": 60},
    4: {"name": "地狱", "size": (15, 15), "mines": 40, "time": 1800, "exp": (3000, 5500), "score": 150},
}
...
# ============================================================================
# 坐骑系统（购买/图鉴/骑乘/升级/入场离场）
# ============================================================================
# 坐骑配置：name -> {base_power(初始战力), stars(灵智星级1-5), value(价值),
#                    reward_min/reward_max(入场一次积分奖励区间), price(市场售价,积分),
#                    desc(图鉴描述)}
# base_power 等差：最低 1 万，之后每个 +2 万（1/3/5/…/23 万）。
MOUNTS = {
    "踏风玄鹿": {
        "base_power": 10_000, "stars": 1, "value": 1_000_000,
        "reward_min": 1_000, "reward_max": 2_000, "price": 600_000,
        "desc": "踏风而行，奔若惊鸿。新手入门坐骑，轻盈迅捷。",
    },
    "踏云追日": {
        "base_power": 30_000, "stars": 1, "value": 3_000_000,
        "reward_min": 2_000, "reward_max": 4_000, "price": 1_500_000,
        "desc": "脚踏祥云，追逐日光。为长途跋涉而生。",
    },
    "北冥冰麟": {
        "base_power": 50_000, "stars": 2, "value": 6_000_000,
        "reward_min": 4_000, "reward_max": 8_000, "price": 3_000_000,
        "desc": "北冥之渊的冰晶麒麟，脚踏冰霜，寒气逼人。",
    },
    "焚天炎驹": {
        "base_power": 70_000, "stars": 2, "value": 10_000_000,
        "reward_min": 6_000, "reward_max": 12_000, "price": 5_000_000,
        "desc": "焚天之火凝聚的神驹，奔行之间烈焰滔天。",
    },
    "紫霄镇世葫": {
        "base_power": 90_000, "stars": 3, "value": 15_000_000,
        "reward_min": 10_000, "reward_max": 20_000, "price": 8_000_000,
        "desc": "紫霄宫中的镇世宝葫，御之可遨游九天。",
    },
    "九天应龙": {
        "base_power": 110_000, "stars": 3, "value": 25_000_000,
        "reward_min": 15_000, "reward_max": 30_000, "price": 12_000_000,
        "desc": "九天之上应运而生的神龙，威震八荒。",
    },
    "撼岳青狮": {
        "base_power": 130_000, "stars": 4, "value": 40_000_000,
        "reward_min": 25_000, "reward_max": 50_000, "price": 18_000_000,
        "desc": "一声吼可撼动山岳的青狮，王者之姿。",
    },
    "玄冰翳凤": {
        "base_power": 150_000, "stars": 4, "value": 60_000_000,
        "reward_min": 35_000, "reward_max": 70_000, "price": 25_000_000,
        "desc": "玄冰覆盖双翼的翳凤，振翅便引万里寒霜。",
    },
    "九曜龙鲤": {
        "base_power": 170_000, "stars": 4, "value": 90_000_000,
        "reward_min": 50_000, "reward_max": 90_000, "price": 35_000_000,
        "desc": "承载九曜星光的龙鲤，跃门化龙指日可待。",
    },
    "炼狱赤魔狮": {
        "base_power": 190_000, "stars": 5, "value": 130_000_000,
        "reward_min": 70_000, "reward_max": 120_000, "price": 45_000_000,
        "desc": "炼狱业火中诞生的赤魔狮，凶焰滔天。",
    },
    "破军铁骑": {
        "base_power": 210_000, "stars": 5, "value": 170_000_000,
        "reward_min": 80_000, "reward_max": 150_000, "price": 60_000_000,
        "desc": "披坚执锐的破军铁骑，踏碎千军。",
    },
    "阴阳镇世": {
        "base_power": 230_000, "stars": 5, "value": 200_000_000,
        "reward_min": 100_000, "reward_max": 200_000, "price": 80_000_000,
        "desc": "调和阴阳、镇守一方的至尊坐骑，天下无双。",
    },
}

# 坐骑低→高排序（图鉴/市场展示用）
MOUNTS_ORDER = [
    "踏风玄鹿", "踏云追日", "北冥冰麟", "焚天炎驹", "紫霄镇世葫", "九天应龙",
    "撼岳青狮", "玄冰翳凤", "九曜龙鲤", "炼狱赤魔狮", "破军铁骑", "阴阳镇世",
]

MOUNT_UPGRADE_COST_DIAMOND = 5000   # 坐骑升级消耗钻石
MOUNT_UPGRADE_POWER_MIN = 500      # 每次升级战力 + 随机下限
MOUNT_UPGRADE_POWER_MAX = 1000     # 每次升级战力 + 随机上限
MOUNT_IDLE_TIMEOUT_MIN = 30        # 无消息 N 分钟自动离场
