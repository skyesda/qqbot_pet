"""AI 意图路由：把自然语言消息翻译为灵契仙途的标准指令。

三层路由中的第 2/3 层（第 1 层为 main.py 中的精确指令匹配）：

- 第 2 层（本地，零延迟）：同义词表 + 正则规则，覆盖常见的自然语言变体，
  如「帮我升级」→「灵宠升级」、「转让给123456 还魂丹 2个」→「转让 123456 还魂丹 2」。
...
# 高危指令：后果不可逆（包括等级重置类的进化/飞升/渡劫），
# 禁止由 AI 路由自动执行，必须玩家手动发送完整指令
DANGEROUS_COMMANDS = {
    "放生宠物",
    "赠送宠物",
    "清空背包",
    ...
    "宠物飞升",
    "宠物渡劫",
    # 修士化别名（灵宠X 同样不可由 AI 自动执行）
    "灵宠进化",
    "灵宠飞升",
    "灵宠渡劫",
}

# 陈述/闲聊句式：玩家在描述已发生的事或聊天，不是在下达指令，
# 直接跳过 AI 路由（如「我都进化了」「我已经升级过了」「他渡劫失败了」）
STATEMENT_RE = re.compile(
    r"(都|已经|已|刚刚|刚才|刚|早就|之前|昨天|上次).{0,12}(了|过)"
    r"|(了|过|完了|好了)$"
    r"|^(他|她|它|你|你们|大家|别人|谁|怎么|为什么|为啥|是不是|能不能|可以吗|吗)"
)
...
复活 用户ID
精力转移 用户ID 精力值
进入副本 副本名称（省略名称则自动打可进的最高档）
挑战神仙 等级（省略等级则自动打可进的最高档）
深渊购买 商品名
深渊祝福 祝福名
提交任务 任务名称
领取任务 任务名称
...
_SYSTEM_PROMPT = (
    "你是QQ群宠物养成游戏『灵契仙途』的指令解析器。"
    "任务：判断玩家消息是否是在【明确请求立即执行某个游戏操作】，若是则翻译为一条标准指令行。"
    "只输出JSON，格式：{\"cmd\": \"指令行\"}；否则输出 {\"cmd\": null}。"
    "不要输出任何解释。指令行的第一个词必须来自指令列表。"
    "严格规则（宁可输出 null，也不要误执行）："
    "1) 陈述已发生的事、表达结果或感叹（如「我都进化了」「我已经升级过了」「终于渡劫成功」）一律输出 null；"
    "2) 提问、讨论玩法、谈论别人或闲聊（如「进化要多少金币」「他飞升了吗」）一律输出 null；"
    "3) 只有玩家用祈使句主动要求现在执行（如「帮我进化」「来一次渡劫」）才输出指令；"
    "4) 消息中含「了/过/已经/都/刚」等完成态词时，默认视为陈述，输出 null；"
    "5) 模棱两可、无法 100% 确定玩家意图时，输出 null。"
    "示例：「我都进化了」→{\"cmd\": null}；「帮我升个级」→{\"cmd\": \"宠物升级\"}；"
    "「昨天签到过了」→{\"cmd\": null}；「签到」→{\"cmd\": \"签到\"}。"
)
...
class AIRouter:
    ...
    async def route(self, text: str, known_commands: set[str]) -> str | None:
        ...
        if local and local.split()[0] in known_commands:
            return local
        # LLM 兜底触发条件：长度合理、疑似宠物相关，且不是陈述/闲聊句式
        if len(text) > MAX_TEXT_LEN or not self.looks_pet_related(text):
            return None
        if STATEMENT_RE.search(text.strip()):
            return None
